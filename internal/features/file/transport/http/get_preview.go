...
func (h *handler) GetPreview(rw http.ResponseWriter, r *http.Request) {
	...
	result, err := h.fileService.GetPreview(r.Context(), fileID, userID)
	if err != nil {
		switch {
		case errors.Is(err, core_errors.ErrNotFound):
			rh.ErrorResponse(err, "Preview or file not found")
			return
		}

		rh.ErrorResponse(err, "Unable to get preview")
		return
	}
	...
	if _, err := io.Copy(rw, result.Content); err != nil {
		log.Error("Write file preview response", zap.Error(err))
	}
}
